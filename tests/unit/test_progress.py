...
class TestProgressConfig:
    ...
    def test_disabled_by_ci_env(self):
        ...
        ) and patch.object(sys.stderr, "isatty", return_value=True):
    ...
    def test_disabled_when_not_tty(self):
        ...
        with patch.dict("os.environ", {}, clear=True) and patch.object(
...
class TestProgressContext:
    ...
    def test_update_increments_progress(self):
        """Update should increment progress counter."""
        with progress_context(total=10, disable=True) as pbar:
            assert pbar.n == 0
            pbar.update(1)
            assert pbar.n == 1
